...
public class DBHandler implements DBTemplate {
    ...
    public List<Book> getBookList() {
        ...
        try(var connection = DBConnection.getConnection()){
            ...
            while (resultSet.next()){
                ...
                var copies = resultSet.getInt("Copies");
                var subject = resultSet.getString("Subject");
                var author = resultSet.getString("Author");

                var date = resultSet.getDate("DatePublished").toLocalDate();
                //LocalDate datePublished = (date == null) ? LocalDate.now() : date.toLocalDate();

                var book = new Book(ISBN, title, subject, author, publisher, copies, date);
                bookList.add(book);
            }
    ...
    public Book getBookByID(Integer ISBN){
        ...
        try(var connection = DBConnection.getConnection()){
            ...
            if (resultSet.next()){
                ...
                var subject = resultSet.getString("Subject");
                var copies = resultSet.getInt("Copies");
                var datePublished = resultSet.getDate("DatePublished").toLocalDate();

                var book = new Book(bookISBN, title, subject, author, publisher, copies, datePublished);
                return book;
            }
    ...
    public void insertBook(Book book) {
        var sql =   """
                    INSERT INTO book (Title, Publisher, Copies, Subject, Author, DatePublished) 
                    VALUES (?, ?, ?, ?, ?, ?);
                    """;
        ...
        public ObservableList<Book> getBooksFromDB(){
            ...
            try(var connection = DBConnection.getConnection()){
                ...
                for(int i = 0; resultSet.next(); i++){
                    ...
                    var book = new Book(ISBN, title, subject, author, publisher, copies, datePublished);
                    bookList.add(book);
